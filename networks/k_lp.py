...
def mini_batch_gd(training_set, validation_set,  weights, biases, betas, gammas, mus_av, vars_av, hyperparams):
    ...
    number_layers = len(hyperparams['HIDDENLAYERS'])+1
    assert number_layers == len(weights)
    v = {}
    s = {}
    eta_adam = hyperparams['ETA_ADAM']

    # initialize v and s
    for l in range(number_layers):
        v["dW" + str(l+1)] = np.zeros(weights[l].shape)
        v["db" + str(l+1)] = np.zeros(biases[l].shape)
        s["dW" + str(l+1)] = np.zeros(weights[l].shape)
        s["db" + str(l+1)] = np.zeros(biases[l].shape)

        if l < len(weights) - 1:
            v["dbeta" + str(l+1)] = np.zeros(betas[l].shape)
            v["dgamma" + str(l+1)] = np.zeros(gammas[l].shape)
            s["dbeta" + str(l+1)] = np.zeros(betas[l].shape)
            s["dgamma" + str(l+1)] = np.zeros(gammas[l].shape)
    ...
    for epoch in range(hyperparams['N_EPOCHS']):
        if(epoch != 0 and epoch % hyperparams['DECAY_FREQ'] == 0):
            if eta_adam > 0.001:
                eta_adam = eta_adam/2
        ...
        for i in range(n//hyperparams['N_BATCH']):
            iteration = i + (epoch*n//hyperparams['N_BATCH']) + 1
            ...
            if hyperparams['OPTIMIZER'] == 'ADAM':
                ...
                for l in range(number_layers):
                    ...
                    v["dW" + str(l+1)] = beta1 * v["dW" + str(l+1)
                                                   ] + (1 - beta1) * weight_gradients[l]
                    v["db" + str(l+1)] = beta1 * v["db" + str(l+1)] + \
                        (1 - beta1) * bias_gradients[l]

                    # correct biases
                    v_corrected["dW" + str(l+1)] = v["dW" +
                                                     str(l+1)] / (1 - beta1 ** iteration)
                    v_corrected["db" + str(l+1)] = v["db" +
                                                     str(l+1)] / (1 - beta1 ** iteration)

                    # moving averages squared gradients:
                    s["dW" + str(l+1)] = beta2 * s["dW" + str(l+1)] + \
                        (1 - beta2) * (np.square(weight_gradients[l]))
                    s["db" + str(l+1)] = beta2 * s["db" + str(l+1)] + \
                        (1 - beta2) * (np.square(bias_gradients[l]))

                    # bias corrected second moment
                    s_corrected["dW" + str(l+1)] = s["dW" +
                                                     str(l+1)] / (1 - beta2 ** iteration)
                    s_corrected["db" + str(l+1)] = s["db" +
                                                     str(l+1)] / (1 - beta2 ** iteration)

                    # Update parameters
                    weights[l] = weights[l] - eta_adam * \
                        v_corrected["dW" + str(l+1)] / \
                        (np.sqrt(s_corrected["dW" + str(l+1)]
                                 ) + hyperparams['EPSILON'])
                    biases[l] = biases[l] - eta_adam * \
                        v_corrected["db" + str(l+1)] / \
                        (np.sqrt(s_corrected["db" + str(l+1)]
                                 ) + hyperparams['EPSILON'])

                    if l < len(weights) - 1:
                        v["dbeta" + str(l+1)] = beta1 * v["dbeta" + str(l+1)] + \
                            (1 - beta1) * beta_gradients[l]
                        v["dgamma" + str(l+1)] = beta1 * v["dgamma" + str(l+1)] + \
                            (1 - beta1) * gamma_gradients[l]
                        v_corrected["dbeta" + str(l+1)] = v["dbeta" +
                                                            str(l+1)] / (1 - beta1 ** iteration)
                        v_corrected["dgamma" + str(l+1)] = v["dgamma" +
                                                             str(l+1)] / (1 - beta1 ** iteration)
                        s["dbeta" + str(l+1)] = beta2 * s["dbeta" + str(l+1)] + \
                            (1 - beta2) * (np.square(beta_gradients[l]))
                        s["dgamma" + str(l+1)] = beta2 * s["dgamma" + str(l+1)] + \
                            (1 - beta2) * (np.square(gamma_gradients[l]))
                        s_corrected["dbeta" + str(l+1)] = s["dbeta" +
                                                            str(l+1)] / (1 - beta2 ** iteration)
                        s_corrected["dgamma" + str(l+1)] = s["dgamma" +
                                                             str(l+1)] / (1 - beta2 ** iteration)
                        betas[l] = betas[l] - eta_adam * \
                            v_corrected["dbeta" + str(l+1)] / \
                            (np.sqrt(s_corrected["dbeta" + str(l+1)]
                                     ) + hyperparams['EPSILON'])
                        gammas[l] = gammas[l] - eta_adam * \
                            v_corrected["dbeta" + str(l+1)] / \
                            (np.sqrt(s_corrected["dbeta" + str(l+1)]
                                     ) + hyperparams['EPSILON'])
...
hps = {
    ...
    'DROP_RATE': 0,
    'HIDDENLAYERS': [20,  20, 20, 20, 20, 20],
    'OPTIMIZER': 'SGD',
    'BETA1': 0.9,
    'BETA2': 0.999,
    'ETA_ADAM': 3e-4,
